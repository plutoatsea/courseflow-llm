...
class WorkflowBaseViewUnconnected extends EditableComponentWithActions{
    ...
    render(){
        ...
        return (
            <div id="workflow-wrapper" class="workflow-wrapper">
                ...
                <NewTabView renderer={this.props.renderer}/>
                {this.getReturnLinks()}
    ...
    updateTabs(){
        ...
        let disabled_tabs=[];
        for(let i=0;i<=5;i++)if(this.allowed_tabs.indexOf(i)<0)disabled_tabs.push(i);
        ...
        // Always enable the new tab
        let current_disabled = $("#sidebar").tabs("option","disabled");
        if(current_disabled.indexOf(5) >= 0) {
            current_disabled = current_disabled.filter(i => i !== 5);
            $("#sidebar").tabs({disabled:current_disabled});
        }
    }
