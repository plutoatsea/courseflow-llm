...
export function newNode(weekPk,position=-1,column=-1,column_type=-1,title=null,description=null,callBackFunction=()=>console.log("success")){
    try{
        const postData = {
        ...
        $.post(post_paths.new_node, postData).done(function(data){
            if(data.action == "posted") callBackFunction(data);
            else fail_function(data.action);
        });
    }catch(err){
